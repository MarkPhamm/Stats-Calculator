...
def create_box_plot():
    n_points = st.slider(
        "Number of data points for box plot:", 100, 5000, 1000, key="box_n_points"
    )
...
def create_line_histogram():
    n_points = st.slider(
        "Number of data points:", 100, 5000, 1000, key="line_hist_n_points"
    )
    n_categories = st.slider(
        "Number of categories:", 1, 3, 2, key="line_hist_n_categories"
    )
...
def create_pie_chart():
    ...
    fig = px.pie(
        ...
        names="Category",
        values="Value",
        title=f"Pie Chart with {n_categories} Categories",
    )
    st.plotly_chart(fig)
...
def create_donut_chart():
    n_categories = st.slider(
        "Number of categories:", 2, 10, 3, key="donut_n_categories"
    )
    ...
    fig.update_layout(
        annotations=[
            dict(
                text=f"<b>{center_metric}: {total_value}</b>",
                x=0.5,
                y=0.5,
                font_size=20,
                showarrow=False,
...
def create_stacked_bar_chart():
    n_categories = st.slider(
        "Number of categories:", 2, 10, 5, key="stacked_bar_n_categories"
    )
    ...
    df = pd.DataFrame(
        {
            "Category": np.repeat(
                [f"Category {i}" for i in range(1, n_categories + 1)], n_series
            ),
            "Series": np.tile(
                [f"Series {i}" for i in range(1, n_series + 1)], n_categories
            ),
...
def create_scatter_plot():
    n_points = st.slider(
        "Number of data points:", 50, 1000, 200, key="scatter_n_points"
    )
    n_categories = st.slider(
        "Number of categories:", 1, 5, 3, key="scatter_n_categories"
    )
...
def create_bubble_chart():
    ...
    n_categories = st.slider(
        "Number of categories:", 1, 5, 3, key="bubble_n_categories"
    )
...
def main():
    ...
    st.write(
        "Distribution charts show how data is spread over a range of values, helping you understand frequency, variation, and central tendency."
    )

    st.subheader("1.1. Histogram")
    st.write(
        "Use a histogram to display the distribution of a single continuous variable. It's useful for identifying patterns like skewness or outliers."
    )
    create_histogram()

    st.subheader("1.2. Box Plot")
    st.write(
        "A box plot helps you visualize the spread and identify outliers in your data. It's good for comparing distributions across different categories."
    )
    create_box_plot()

    st.subheader("1.3. Line Histogram")
    st.write(
        "Use a line histogram to display the distribution of a single continuous variable with a density plot. It's useful for visualizing the shape of the distribution and comparing distributions across different categories."
    )
    ...
    st.write(
        "Use a bar chart to compare different categories or groups. This is ideal for comparing discrete items or categorical data."
    )
    create_bar_chart()

    st.subheader("2.2. Line Chart")
    st.write(
        "A line chart is used to visualize trends over time. It's great for showing changes and patterns in continuous data."
    )
    create_line_chart()
    ...
    st.write(
        "A pie chart shows the relative size of parts to a whole. It's effective for showing proportions, but avoid using it for too many categories (<=5)."
    )
    create_pie_chart()

    st.subheader("3.2. Donut Chart")
    st.write(
        "A donut chart is similar to a pie chart but has a hole in the middle. This design allows for additional metrics or information to be displayed in the center of the donut. While both charts show proportions, the donut chart can provide more context by incorporating additional details in the center."
    )
    create_donut_chart()

    st.subheader("3.3. Stacked Bar Chart")
    st.write(
        "A stacked bar chart shows the total value of different categories while also indicating the proportion of each subcategory within the whole."
    )
    ...
    st.write(
        "A scatter plot is used to show the relationship between two continuous variables. It's helpful for spotting correlations or clusters."
    )
    create_scatter_plot()

    st.subheader("4.2. Bubble Chart")
    st.write(
        "A bubble chart is a variation of a scatter plot where the size of the bubbles represents a third variable. It's useful for adding more dimensions to the analysis."
    )
    create_bubble_chart()
