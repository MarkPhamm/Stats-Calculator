...
def plot_normal_distribution(
    mean, std_dev, x, y=None, sample_size=None, population=True
):
    ...
    if y is None:
        ...
        ax.fill_between(
            ...
            0,
            y_vals,
            where=((x_vals >= x) & (x_vals <= y)),
            color="purple",
            alpha=0.3,
            label="Area between x and y",
        )
        ax.fill_between(
            x_vals,
            0,
...
def normal_distribution():
    ...
    st.latex(
        r"\text{PDF}(x) = \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{(x-\mu)^2}{2\sigma^2}}"
    )
    st.latex(
        r"\text{CDF}(x) = P(X \leq x) = \int_{-\infty}^{x} \frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{(t-\mu)^2}{2\sigma^2}} dt"
    )
    ...
    if y is None:
        ...
        st.success(
            f"Area to the left of x: {area_left:.4f} and Area to the right of x: {area_right:.4f}"
        )
    else:
        ...
        st.success(
            f"Area to the left of x: {area_left_x:.4f} and Area to the right of x: {area_right_x:.4f}"
        )
        st.success(
            f"Area to the left of y: {area_left_y:.4f} and Area to the right of y: {area_right_y:.4f}"
        )
...
def inverse_normal_distribution():
    ...
    std_dev = st.number_input("Enter the Standard Deviation (σ)", value=1.0, step=0.1)
    area_left = st.slider(
        "Enter the percentage of area to the left (as %)",
        min_value=0.01,
        max_value=99.99,
        value=50.0,
        step=0.01,
    ...
    st.success(
        f"The value of x such that {area_left:.2f}% of the distribution is to the left is: {x:.4f}"
    )
...
def main():
    calculation_type = st.sidebar.radio(
        "Choose calculation type:", ["Normal", "Inverse"]
    )
