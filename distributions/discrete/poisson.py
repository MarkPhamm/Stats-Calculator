...
def plot_poisson_distribution(lam: float, x_min: int, x_max: int) -> None:
    ...
    x_vals = np.arange(
        0, x_max + 0.5 * x_max
    )  # Adjusted to extend the range to x_max + 1
    y_vals = poisson.pmf(x_vals, lam)

    fig, ax = plt.subplots()
    ax.plot(
        x_vals, y_vals, marker="o", label="Poisson Distribution", alpha=0.7
    )  # Changed to line plot with markers
...
def main():
    ...
    lam = st.number_input("Enter the Rate (λ)", value=0.0, step=0.1, min_value=0.0)
    x_min = st.number_input(
        "Enter the Minimum Number of Occurrences (x_min)", value=0, step=1, min_value=0
    )
    x_max = st.number_input(
        "Enter the Maximum Number of Occurrences (x_max)",
        value=10,
        step=1,
        min_value=x_min,
    )
    ...
    st.success(
        f"Percentage of occurrences between {x_min} and {x_max}: {total_prob * 100:.2f}%"
    )

    st.header("Calculate P(X = x) for specific values")
    x_values_input = st.text_input(
        "Enter values of x (separated by commas)", value="0, 1, 2"
    )
    ...
    try:
        ...
        st.error(
            "Invalid input for x values. Please enter integers separated by commas."
        )
