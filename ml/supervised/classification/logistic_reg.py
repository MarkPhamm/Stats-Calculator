...
def main():
        ...
        **Formula:**
        ...
        with col_desc:
            ...
            m_val = st.slider(
                ...
                10.0,
                1.0,
                help="Controls how quickly the probability changes.",
            )
            b_val = st.slider(
                "Intercept (b) - Shift",
                -10.0,
                10.0,
                0.0,
                help="Shifts the curve left or right.",
            )
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV or Excel File", type=["csv", "xlsx"], key="logreg_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Variable (Label 0/1)", df.columns, key="log_y"
            )

            # Validate Target (Should be roughly binary for this demo)
            if df[target_col].nunique() > 2:
                st.warning(
                    "⚠️ The selected target has more than 2 unique classes. The model will run in Multinomial mode."
                )
            ...
            feature_cols = st.multiselect(
                "Select Feature Variables (X)", numeric_cols, key="log_x"
            )
            ...
            if feature_cols and st.button("Train Logistic Model"):
                ...
                with col_coef:
                    ...
                    coef_df = pd.DataFrame(
                        {"Feature": feature_cols, "Weight": model.coef_[0]}
