...
def main():
    ...
    with tab1:
        ...
        with col_anim:
            ...
            X, _ = make_blobs(
                n_samples=300, centers=n_centers, cluster_std=0.60, random_state=0
            )
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV File", type=["csv"], key="kmeans_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            selected_features = st.multiselect(
                "Select exactly 2 Features for Visualization", df_numeric.columns
            )
            ...
            if len(selected_features) == 2:
                ...
                st.write(
                    "The 'Elbow' of the graph represents the optimal number of clusters."
                )
                ...
                final_k = st.number_input(
                    "Choose K based on the graph above",
                    min_value=2,
                    max_value=10,
                    value=3,
                )
                ...
                if st.button("Run K-Means Clustering"):
                    ...
                    st.metric(
                        "Silhouette Score",
                        f"{sil_score:.4f}",
                        help="Close to 1 is good, close to -1 is bad.",
                    )
                ...
                st.warning(
                    "Please select exactly 2 features to proceed with 2D visualization."
                )
