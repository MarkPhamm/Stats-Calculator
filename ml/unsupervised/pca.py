...
def main():
    ...
    with tab1:
        ...
        st.caption(
            "The long red arrow is the First Principal Component (PC1) - it captures the most information."
        )
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="pca_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            features = st.multiselect(
                "Select Features to Analyze (Choose > 2)", numeric_cols
            )

            # Optional: Select a column for coloring points
            color_col = st.selectbox(
                "Select Label for Coloring (Optional)", ["None"] + list(df.columns)
            )
            ...
            if len(features) >= 2 and st.button("Run PCA"):
                ...
                st.info(
                    f"Explained Variance: PC1 ({var_ratio[0]:.2%}) + PC2 ({var_ratio[1]:.2%}) = Total {(sum(var_ratio)):.2%}"
                )
                ...
                if color_col != "None":
                    sns.scatterplot(
                        data=pca_df, x="PC1", y="PC2", hue="Label", alpha=0.7, ax=ax
                    )
                else:
