...
def main():
    ...
    def plot_normal_distribution_with_moe(center, moe, std_error, confidence_level):
        ...
        ax.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Lower Bound = {ci_lower:.2f}",
        )
        ax.axvline(
            x=ci_upper,
            color="red",
            linestyle="--",
            label=f"Upper Bound = {ci_upper:.2f}",
        )
        ax.axvline(
            x=center, color="green", linestyle="-", label=f"Center = {center:.2f}"
        )

        ax.set_title(
            f"Normal Distribution with {confidence_level}% Confidence Interval"
        )
        ax.set_xlabel("Value")
    ...
    st.latex(
        r"\text{MOE for Mean (known } \sigma\text{)} = Z \cdot \frac{\sigma}{\sqrt{n}}"
    )
    st.latex(
        r"\text{MOE for Mean (unknown } \sigma\text{)} = t \cdot \frac{s}{\sqrt{n}}"
    )
    ...
    confidence_level = st.slider(
        "Select Confidence Level (%)", min_value=50, max_value=99, value=95, step=1
    )

    if calculation_type == "Mean":
        sigma_known = st.selectbox(
            "Is Population Standard Deviation (σ) known?", ["Yes", "No"]
        )
        ...
        if sigma_known == "Yes":
            ...
            st.success(
                f"Confidence Interval: ({sample_mean - moe:.4f}, {sample_mean + moe:.4f})"
            )

            plot_normal_distribution_with_moe(
                sample_mean, moe, std_error, confidence_level
            )

            st.success(
                f"With {confidence_level}% confidence, the population mean is estimated to be between {sample_mean - moe:.4f} and {sample_mean + moe:.4f}."
            )
        ...
        st.success(f"Margin of Error (MOE): {moe:.4f}")
        st.success(
            f"Confidence Interval: ({proportion - moe:.4f}, {proportion + moe:.4f})"
        )

        plot_normal_distribution_with_moe(proportion, moe, std_error, confidence_level)

        st.write(
            f"With {confidence_level}% confidence, the population proportion is estimated to be between {proportion - moe:.4f} and {proportion + moe:.4f}."
        )
