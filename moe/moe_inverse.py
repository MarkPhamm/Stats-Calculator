...
def main():
    ...
    st.latex(
        r"\text{Sample Size for Mean (known } \sigma\text{)}: n = \left(\frac{Z \cdot \sigma}{MOE}\right)^2"
    )
    st.latex(
        r"\text{Sample Size for Mean (unknown } \sigma\text{)}: n = \left(\frac{t \cdot s}{MOE}\right)^2"
    )
    st.latex(r"\text{Sample Size for Proportion}: n = \frac{Z^2 \cdot p(1 - p)}{MOE^2}")

    calculation_type = st.selectbox(
        "Calculate Sample Size for:", ["Mean", "Proportion"]
    )
    ...
    if calculation_type == "Mean":
        ...
        if sigma_known == "Yes":
            ...
            t_score = get_t_score(
                confidence_level, 30
            )  # Assuming a default sample size for t-score
