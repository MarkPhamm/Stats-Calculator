...
def main():
    ...
    st.write("Test whether there is a statistically significant difference between the means of three or more independent groups.")

    st.sidebar.markdown("---")
    st.sidebar.header("Data Input")
    
    st.info("Please enter numerical values separated by commas (e.g., 10.5, 12.1, 14.2).")
    ...
    if st.button("Run ANOVA Test"):
        try:
            ...
            df_vis = pd.DataFrame({
                ...
                'Group': ['Group A']*len(data_a) + ['Group B']*len(data_b) + ['Group C']*len(data_c)
            })
            ...
            st.error("Input Error: Please ensure all inputs are valid numbers separated by commas.")