...
def plot_chi_square_distribution(chi2_stat, dof=None):
    ...
    if dof:
        plt.plot(
            x,
            stats.chi2.pdf(x, df=dof),
            "r-",
            label=f"Chi-Square Distribution (df={dof})",
        )
    ...
    plt.axvline(
        x=chi2_stat, color="b", linestyle="--", label=f"Test Statistic: {chi2_stat:.4f}"
    )
...
def display_results(chi2_stat, p_value, dof, expected=None, alpha=0.05):
    ...
    st.success(
        f"**Critical Value ({(1 - alpha) * 100:.0f}% confidence):** {critical_value:.4f}"
    )

    if chi2_stat > critical_value:
        st.success(
            "**Conclusion:** Reject the null hypothesis. There is significant evidence of a difference or dependence."
        )
    else:
        st.error(
            "**Conclusion:** Fail to reject the null hypothesis. There is not enough evidence to conclude a difference or dependence."
        )
...
def main():
    ...
    test_type = st.sidebar.radio(
        "Choose Chi-Square Test", ("Goodness of Fit", "Test of Independence")
    )
...
def goodness_of_fit_test():
    ...
    st.latex(
        r"\text{H₁: The observed distribution does not fit the expected distribution}"
    )
    st.latex(r"\chi^2 = \sum \frac{(O_i - E_i)^2}{E_i}")

    num_categories = st.number_input(
        "Number of categories", min_value=1, value=3, step=1
    )

    observed_df = pd.DataFrame({"Observed": [0.0] * num_categories})
    expected_df = pd.DataFrame({"Expected": [0.0] * num_categories})

    observed_values = st.data_editor(
        observed_df, use_container_width=True, key="observed_data"
    )
    expected_values = st.data_editor(
        expected_df, use_container_width=True, key="expected_data"
    )

    alpha = st.slider(
        "Significance Level (α)", min_value=0.01, max_value=0.20, value=0.05, step=0.01
    )
    ...
    if st.button("Run Goodness of Fit Test"):
        if observed_values.empty or expected_values.empty:
            ...
            if len(observed) != len(expected):
                st.error(
                    "Length of observed and expected frequencies must be the same."
                )
            elif not np.isclose(np.sum(observed), np.sum(expected), rtol=1e-8):
                st.error(
                    "The sum of observed frequencies must match the sum of expected frequencies."
                )
            else:
...
def independence_test():
    ...
    contingency_df = pd.DataFrame(
        np.zeros((num_rows, num_cols)), columns=[f"Col{i + 1}" for i in range(num_cols)]
    )
    contingency_table = st.data_editor(
        contingency_df, use_container_width=True, key="contingency_table"
    )
    ...
    if st.button("Run Test of Independence"):
        if contingency_table.empty:
            ...
            chi2_stat, p_value, dof, expected = chi_square_test_of_independence(
                contingency_table
            )
