...
def main():
    ...
    with tab1:
        ...
        **Update rule:**  $F_m(x) = F_{m-1}(x) + \eta \cdot h_m(x)$
        ...
        st.caption(
            "Adjust parameters and watch how train/test accuracy evolves across boosting rounds."
        )
        ...
        with col_ctrl1:
            ...
            st.info(
                "Lower LR = smoother curve that often peaks later. High LR = fast but may overfit."
            )

        with col_plot1:
            X1, y1 = make_classification(
                n_samples=n_samp, n_features=10, random_state=42
            )
        ...
        with col_plot2:
            if selected_lrs:
                X2, y2 = make_classification(
                    n_samples=400, n_features=10, random_state=42
                )
                ...
                for lr in sorted(selected_lrs):
                    ...
                    gb2.fit(X2_tr, y2_tr)
                    scores = [
                        accuracy_score(y2_te, p) for p in gb2.staged_predict(X2_te)
                    ]
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="gb_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Column", df.columns, key="gb_target"
            )
            ...
            feature_cols = st.multiselect(
                "Select Feature Columns", numeric_cols, key="gb_features"
            )
            ...
            if feature_cols and st.button("🚀 Train Gradient Boosting"):
                ...
                if task == "Classification":
                    ...
                    staged = [
                        accuracy_score(y_te, p) for p in model.staged_predict(X_te)
                    ]
                    fig_lc, ax_lc = plt.subplots()
                    ax_lc.plot(staged, color="#00AEEF")
                    ax_lc.set_xlabel("Boosting Stage")
