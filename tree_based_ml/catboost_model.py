...
def main():
    ...
    with tab1:
        ...
        st.caption(
            "CatBoost uses symmetric (oblivious) trees — every node at the same depth shares the same split."
        )
        col_ctrl1, col_plot1 = st.columns([1, 2])
        with col_ctrl1:
            n_iter_1 = st.slider("iterations", 20, 300, 100, step=10, key="cb_s1_iter")
            lr_1 = st.slider(
                "Learning Rate", 0.01, 0.5, 0.05, step=0.01, key="cb_s1_lr"
            )
            ...
            st.info(
                "Default depth=6 is usually a sweet spot. Deeper trees can overfit with CatBoost's symmetric structure."
            )

        with col_plot1:
            X1, y1 = make_classification(
                n_samples=n_samp_1, n_features=12, random_state=42
            )
        ...
        st.caption(
            "Train and test log-loss tracked across all iterations via get_evals_result()."
        )
        col_ctrl2, col_plot2 = st.columns([1, 2])
        with col_ctrl2:
            n_iter_2 = st.slider("iterations", 20, 500, 200, step=10, key="cb_s2_iter")
            lr_2 = st.slider(
                "Learning Rate", 0.01, 0.5, 0.05, step=0.01, key="cb_s2_lr"
            )
            depth_2 = st.slider("depth", 1, 10, 6, key="cb_s2_d")
            l2_leaf = st.slider("l2_leaf_reg", 0.0, 10.0, 3.0, step=0.5, key="cb_s2_l2")
            st.info(
                "l2_leaf_reg is CatBoost's L2 regularisation. Increase it to flatten the train curve and close the gap."
            )
    ...
    with tab2:
        ...
        st.info(
            "💡 CatBoost handles categorical columns natively — no encoding required!"
        )

        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="cb_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Column", df.columns, key="cb_target"
            )
            all_cols = [c for c in df.columns if c != target_col]
            feature_cols = st.multiselect(
                "Select Feature Columns", all_cols, key="cb_features"
            )
            ...
            if feature_cols and st.button("🚀 Train CatBoost"):
                ...
                sns.barplot(
                    data=imp_df, x="Importance", y="Feature", palette="Reds_r", ax=ax_i
                )
