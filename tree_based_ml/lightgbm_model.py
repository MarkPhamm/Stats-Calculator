...
def main():
    ...
    with tab1:
        ...
        st.caption("num_leaves controls model complexity in LightGBM's leaf-wise growth.")
        col_ctrl1, col_plot1 = st.columns([1, 2])
        with col_ctrl1:
            n_est_1    = st.slider("n_estimators",     10, 300, 100, step=10, key="lgb_s1_n")
            lr_1       = st.slider("Learning Rate",    0.01, 0.5, 0.05, step=0.01, key="lgb_s1_lr")
            n_samp_1   = st.slider("Samples",          200, 1000, 500, step=50,    key="lgb_s1_s")
            max_leaves = st.slider("Max num_leaves to test", 4, 256, 128, step=4,  key="lgb_s1_max")
            st.info("Beyond a sweet spot, more leaves overfit. Rule: num_leaves < 2^max_depth.")

        with col_plot1:
            X1, y1 = make_classification(n_samples=n_samp_1, n_features=10, random_state=42)
            ...
            for nl in leaf_range:
                m = lgb.LGBMClassifier(n_estimators=n_est_1, num_leaves=nl,
                                        learning_rate=lr_1, verbose=-1, random_state=42)
                m.fit(X1_tr, y1_tr)
                accs.append(accuracy_score(y1_te, m.predict(X1_te)))

            fig1, ax1 = plt.subplots(figsize=(7, 3.5))
            ax1.plot(leaf_range, accs, marker="o", color="#2ECC71")
            ax1.set_xlabel("num_leaves"); ax1.set_ylabel("Test Accuracy")
        ...
        st.caption("See where the test loss bottoms out — that's where early stopping would fire.")
        col_ctrl2, col_plot2 = st.columns([1, 2])
        with col_ctrl2:
            n_est_2    = st.slider("n_estimators",    20, 500, 200, step=10, key="lgb_s2_n")
            lr_2       = st.slider("Learning Rate",   0.01, 0.3, 0.05, step=0.01, key="lgb_s2_lr")
            ...
            st.info("The gap between train and test loss reveals overfitting. Raise min_child_samples to reduce it.")
        ...
        with col_plot2:
            ...
            m2 = lgb.LGBMClassifier(n_estimators=n_est_2, learning_rate=lr_2,
                                     num_leaves=num_leaves_2, min_child_samples=min_child_2,
            ...
            if best_it and best_it > 0:
                ax2.axvline(best_it - 1, color="orange", linestyle="--", alpha=0.8,
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader("Upload CSV/Excel", type=["csv", "xlsx"], key="lgb_upload")
        ...
        if uploaded_file is not None:
            ...
            task = st.radio("Task Type", ["Classification", "Regression"], horizontal=True, key="lgb_task")
            target_col = st.selectbox("Select Target Column", df.columns, key="lgb_target")
            numeric_cols = df.select_dtypes(include=["float64", "int64"]).columns.tolist()
            if target_col in numeric_cols:
                numeric_cols.remove(target_col)
            feature_cols = st.multiselect("Select Feature Columns", numeric_cols, key="lgb_features")
            ...
            if feature_cols and st.button("🚀 Train LightGBM"):
                ...
                sns.barplot(data=imp_df, x="Importance", y="Feature", palette="Greens_r", ax=ax_i)
