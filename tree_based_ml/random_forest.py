...
def main():
    ...
    with tab1:
        ...
        with col_ctrl1:
            ...
            max_trees = st.slider(
                "Max trees to test", 10, 200, 100, step=10, key="rf_s1_t"
            )
        ...
        with col_plot1:
            X1, y1 = make_classification(
                n_samples=n_samples_1, n_features=n_features_1, random_state=42
            )
            ...
            for n in est_range:
                rf = RandomForestClassifier(
                    n_estimators=n, max_depth=max_d_1, random_state=42, n_jobs=-1
                )
                rf.fit(X1_tr, y1_tr)
            ...
            ax1.set_title(
                f"Error vs. Trees  (depth={max_d_1}, {n_features_1} features)"
            )
            ax1.legend()
            ax1.grid(alpha=0.3)
            plt.tight_layout()
        ...
        with col_ctrl2:
            ...
            n_trees_imp = st.slider(
                "n_estimators", 10, 200, 100, step=10, key="rf_s2_n"
            )
            st.info(
                "Blue bars = informative features. Grey = noise. Adjust ratio to see MDI shift."
            )
        ...
        with col_plot2:
            X2, y2 = make_classification(
                ...
                n_informative=min(n_informative, n_total_feats),
                random_state=42,
            )
            feat_names = [f"F{i + 1}" for i in range(n_total_feats)]
            rf2 = RandomForestClassifier(
                n_estimators=n_trees_imp, random_state=42, n_jobs=-1
            )
            rf2.fit(X2, y2)
            imp = pd.Series(rf2.feature_importances_, index=feat_names).sort_values(
                ascending=True
            )
            ...
            ax2.set_title(
                f"Feature Importances  ({n_informative} informative highlighted in blue)"
            )
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="rf_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Column", df.columns, key="rf_target"
            )
            ...
            feature_cols = st.multiselect(
                "Select Feature Columns", numeric_cols, key="rf_features"
            )
            ...
            if feature_cols and st.button("🚀 Train Random Forest"):
                ...
                sns.barplot(
                    data=imp_df, x="Importance", y="Feature", palette="Blues_r", ax=ax_i
                )
                st.pyplot(fig_i)
                plt.close(fig_i)
