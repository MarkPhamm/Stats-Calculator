...
def main():
    ...
    with tab1:
        ...
        with col_plot1:
            ...
            m1.fit(
                X1_tr, y1_tr, eval_set=[(X1_tr, y1_tr), (X1_te, y1_te)], verbose=False
            )
            res = m1.evals_result()

            fig1, ax1 = plt.subplots(figsize=(7, 3.5))
            ax1.plot(
                res["validation_0"]["logloss"], label="Train Loss", color="#00AEEF"
            )
            ...
            ax1.set_title(
                f"XGBoost Loss Curves  LR={lr_1}, depth={depth_1}, sub={subsamp}"
            )
        ...
        st.caption(
            "Slide the regularisation strength and see how test accuracy responds."
        )
        col_ctrl2, col_plot2 = st.columns([1, 2])
        with col_ctrl2:
            n_informative_2 = st.slider(
                "Informative features", 2, 15, 5, key="xgb_s2_inf"
            )
            n_est_2 = st.slider("n_estimators", 20, 200, 80, step=10, key="xgb_s2_n")
            alpha_max = st.slider("Max α / λ to test", 1, 20, 10, key="xgb_s2_max")
            st.info(
                "Strong regularisation helps on noisy data but can underfit if too aggressive."
            )
        ...
        with col_plot2:
            ...
            ax2.plot(
                alphas, l1_accs, marker="o", label="L1 (reg_alpha)", color="#00AEEF"
            )
            ax2.plot(
                alphas, l2_accs, marker="s", label="L2 (reg_lambda)", color="#9B59B6"
            )
            ax2.set_xlabel("Regularisation Strength")
            ax2.set_ylabel("Test Accuracy")
            ax2.set_title(
                f"L1 vs L2  ({n_informative_2} informative / 20 total features)"
            )
            ax2.legend()
            ax2.grid(alpha=0.3)
            plt.tight_layout()
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="xgb_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Column", df.columns, key="xgb_target"
            )
            ...
            feature_cols = st.multiselect(
                "Select Feature Columns", numeric_cols, key="xgb_features"
            )
            ...
            with st.expander("⚙️ Regularisation"):
                ...
                reg_alpha = cr1.number_input(
                    "reg_alpha (L1)", 0.0, 10.0, 0.0, key="xgb_alpha"
                )
                reg_lambda = cr2.number_input(
                    "reg_lambda (L2)", 0.0, 10.0, 1.0, key="xgb_lambda"
                )
            ...
            if feature_cols and st.button("🚀 Train XGBoost"):
                ...
                if task == "Classification":
                    ...
                    model.fit(
                        X_tr, y_tr, eval_set=[(X_tr, y_tr), (X_te, y_te)], verbose=False
                    )
