...
def main():
    ...
    with tab1:
        ...
        **Key formula:** learner weight $\alpha_m = \frac{1}{2}\ln\!\left(\frac{1-\varepsilon_m}{\varepsilon_m}\right)$
        ...
        st.caption(
            "Increase estimators to see AdaBoost refine its boundary on the moons dataset."
        )
        col_ctrl1, col_plot1 = st.columns([1, 2])
        with col_ctrl1:
            n_est_1 = st.slider("n_estimators", 1, 300, 50, step=5, key="ada_s1_n")
            noise_1 = st.slider(
                "Noise level", 0.0, 0.4, 0.25, step=0.05, key="ada_s1_noise"
            )
            ...
            st.info(
                "Too many estimators on noisy data → overfitting. Watch the boundary become jagged."
            )
        ...
        with col_plot1:
            ...
            ax1.set_title(
                f"Decision Boundary  (n={n_est_1}, LR={lr_1}, acc={acc1:.2%})"
            )
            plt.tight_layout()
            st.pyplot(fig1)
            plt.close(fig1)
        ...
        st.caption(
            "Watch how misclassified samples gain weight forcing the next learner to focus on them."
        )
        ...
        with col_ctrl2:
            ...
            st.info(
                "Misclassified samples (tall bars) get boosted weight so the next stump must focus on them."
            )
        ...
        with col_plot2:
            ...
            fig2, axes = plt.subplots(
                1, n_rounds, figsize=(min(14, n_rounds * 2.5), 3), sharey=True
            )
            if n_rounds == 1:
                axes = [axes]
    ...
    with tab2:
        ...
        uploaded_file = st.file_uploader(
            "Upload CSV/Excel", type=["csv", "xlsx"], key="ada_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            target_col = st.selectbox(
                "Select Target Column", df.columns, key="ada_target"
            )
            ...
            feature_cols = st.multiselect(
                "Select Feature Columns", numeric_cols, key="ada_features"
            )
            ...
            if feature_cols and st.button("🚀 Train AdaBoost"):
                ...
                if task == "Classification":
                    ...
                    ax_w.bar(
                        range(len(model.estimator_weights_)),
                        model.estimator_weights_,
                        color="#9B59B6",
                        alpha=0.8,
                    )
                    ax_w.set_xlabel("Estimator Index")
                    ax_w.set_ylabel("Weight (α)")
                    ...
                    model = AdaBoostRegressor(
                        n_estimators=n_est, learning_rate=lr_val, random_state=42
                    )
                    model.fit(X_tr, y_tr)
                    y_pred = model.predict(X_te)
                    c1, c2 = st.columns(2)
